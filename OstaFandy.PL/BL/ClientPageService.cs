...
namespace OstaFandy.PL.BL
{
    public class ClientPageService : IClientPageService
    {
        ...
        #region approve job state change
        public bool ApproveJobStatusChange(int jobId, string approvedStatus, int clientUserId)
        {
            try
            {
                ...
                n.RelatedEntityType == "JobAssignment" &&
                n.RelatedEntityId == jobId);
                ...
                {
                    _logger.LogWarning($"Client user ID {clientUserId} is not authorized to approve job assignment ID {jobId}. Expected client ID: {booking.ClientId}");
                    return false;
                }
